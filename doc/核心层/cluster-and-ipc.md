...
在框架里，我们采用 [graceful] 和 [cmos-cluster] 两个模块配合实现上面的逻辑。
...
4. Agent 和普通 App Worker 挂载的 API 不完全一样，如何识别差异可查看[框架文档]
...
Master 进程的稳定性是极高的，线上运行时我们只需要在后台运行通过 `cmos.startCluster` 启动的 Master 进程就可以了，不再需要使用 [pm2] 等进程守护模块。
...
```bash
# 后台运行 Master 进程
$ CMOS_SERVER_ENV=prod nohup node dispatch.js&
```
...
Worker 进程负责处理真正的用户请求和[定时任务](../basics/schedule.md)的处理。而 cmos 的定时任务也提供了只让一个 Worker 进程运行的能力，**所以能够通过定时任务解决的问题就不要放到 Agent 上执行**。
...
- `agent.messenger.sendRandom(action, data)`:
   - app 上没有该方法（现在 cmos 的实现是等同于 sentToAgent）
...
// app.js
module.exports = app => {
  // 注意，只有在 cmos-ready 事件拿到之后才能发送消息
  app.messenger.once('cmos-ready', () => {
...
上面的示例中提到，需要等 `cmos-ready` 消息之后才能发送消息。只有在 Master 确认所有的 Agent 进程和 Worker 进程都已经成功启动（并 ready）之后，才会通过 messenger 发送 `cmos-ready` 消息给所有的 Agent 和 Worker，告知一切准备就绪，IPC 通道可以开始使用了。
...
在实际项目中，我们可以采用方案一用于兜底，结合方案三或者方案二的一种用于提升数据更新的实时性。而在这个示例中，我们会通过 IPC + [定时任务](schedule.md)来同时实现这三种缓存更新方案。
