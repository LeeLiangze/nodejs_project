...
在框架中内置了安全插件 [cmos-security]()， 提供了默认的安全实践。
...
match 和 ignore 使用方法和格式与[中间件通用配置](middleware.md#match和ignore)一致。
...
当网站需要直接输出用户输入的结果时，请务必使用 `helper.escape()` 包裹起来，如在 [cmos-view-nunjucks] 里面就覆盖掉了内置的 `escape`。
...
为了防范 [BREACH 攻击](http://breachattack.com/)，通过同步方式渲染到页面上的 CSRF token 在每次请求时都会变化，[cmos-view-nunjucks] 等 View 插件会自动对 Form 进行注入，对应用开发者无感知。
...
因此，请各位使用 cmos 框架开发网站的开发者，务必推动自己的网站升级到 HTTPS。
