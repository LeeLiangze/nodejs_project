...
框架内置 [cmos-view] 作为模板解决方案，并支持多模板渲染，每个模板引擎都以插件的方式引入，但保持渲染的 API 一致。如果想更深入的了解，可以查看[模板插件开发](view-plugin.md)。

以 View 插件 [cmos-view-nunjucks] 为例

## 引入 view 插件

```bash
$ npm i cmos-view-nunjucks --save
```
...
[cmos-view] 提供了 `config.view` 通用配置
...
const data = { user: 'cmos' };

// will auto merge `data` to `this.locals`, output: cmos - showcase
...
在模板中可以直接使用 `helper` 上注册的方法，具体可以参见[扩展](extend.md)。
...
框架内置的 [cmos-security] 插件，为我们提供了常见的安全辅助函数，包括 `helper.shtml / surl / sjs` 等等等，强烈建议阅读下[安全](./security.md)。
