...
在前面的[多进程模型章节](../核心层/cluster-and-ipc.md)中，我们详细讲述了框架的多进程模型，其中适合使用 Agent 进程的有一类常见的场景：一些中间件客户端需要和服务器建立长连接，理论上一台服务器最好只建立一个长连接，但多进程模型会导致 n 倍（n = Worker 进程数）连接被创建。
...
class APIClient extends Base {
  constructor(options) {
    ...
    // options.cluster 用于给 Cmos 的插件传递 app.cluster 进来
