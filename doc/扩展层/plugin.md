...
2. 插件需要在 `package.json` 中的 `cmosPlugin` 节点指定插件特有的信息

    - `{String} name` - 插件名（必须配置），具有唯一性，配置依赖关系时会指定依赖插件的 name。
    - `{Array} dependencies` - 当前插件强依赖的插件列表（如果依赖的插件没找到，应用启动失败）。
    - `{Array} optionalDependencies` - 当前插件的可选依赖插件列表（如果依赖的插件未开启，只会 warning，不会影响应用启动）。
    - `{Array} env` - 只有在指定运行环境才能开启，具体有哪些环境可以参考[运行环境](../基础层/env.md)。此配置是可选的，一般情况下都不需要配置。
...
1. 首先在 `app/middleware` 目录下定义好中间件实现
  ...
  module.exports = (options, app) => {
    ...
    app.loggers.coreLogger.info('[cmos-static] starting static serve %s -> %s', options.prefix, options.dir);
...
许多插件的目的都是将一些已有的服务引入到框架中，如 [cmos-mysql], [cmos-oss]。他们都需要在 app 上创建对应的实例。而在开发这一类的插件时，我们发现存在一些普遍性的问题：
...
比如定义了一个开发环境使用的插件 `cmos-dev`，只希望在本地环境加载，可以如下定义
...
你可以直接通过 [cmos-init] 选择 [plugin][cmos-boilerplate-plugin] 脚手架来快速上手。

```bash
$ cmos-init cmos-xxx --type=plugin
$ cd cmos-xxx
$ npm i
$ npm test
```
...
- 命名规范
  - `npm` 包名以 `cmos-` 开头，且为全小写，例如：`cmos-xx`。比较长的词组用中划线：`cmos-foo-bar`
  - 对应的插件名使用小驼峰，小驼峰转换规则以 `npm` 包名的中划线为准 `cmos-foo-bar` => `fooBar`
  - 对于可以中划线也可以不用的情况，不做强制约定，例如：userservice(cmos-userservice) 还是 user-service(cmos-user-service) 都可以
- `package.json` 书写规范
  - 按照上面的文档添加 `cmosPlugin` 节点
  - 在 `keywords` 里加上 `cmos`、`cmos-plugin`、`cmosPlugin` 等关键字，便于索引

  ```json
  {
    "name": "cmos-view-nunjucks",
    "version": "1.0.0",
    "description": "view plugin for cmos",
    "cmosPlugin": {
      "name": "nunjucks",
      "dep": [
        "security"
...
cmos 是通过 `cmosPlugin.name` 来定义插件名的，只在应用或框架具备唯一性，也就是说**多个 npm 包可能有相同的插件名**，为什么这么设计呢？

cmos 可以使用这种特性来做适配器。比如[模板开发规范](./view-plugin.md#插件命名规范)定义的插件名为 view，而存在 `cmos-view-nunjucks` 等插件，使用者只需要更换插件和修改模板，不需要动 Controller， 因为所有的模板插件都实现了相同的 API。
