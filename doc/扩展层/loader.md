...
cmos 在 Koa 的基础上进行增强最重要的就是基于一定的约定，根据功能差异将代码放到不同的目录下管理，对整体团队的开发成本提升有着明显的效果。Loader 实现了这套约定，并抽象了很多底层 API 可以进一步扩展。

## 应用、框架和插件

cmos 是一个底层框架，应用可以直接使用，但 cmos 本身的插件比较少，应用需要自己配置插件增加各种特性，比如 MySQL。
...
当应用达到一定数量，我们会发现大部分应用的配置都是类似的，这时可以基于 cmos 扩展出一个框架，应用的配置就会简化很多。
...
- 框架是一个启动器（默认就是 Cmos），必须有它才能运行起来。框架还是一个封装器，将插件的功能聚合起来统一提供，框架也可以配置插件。
...
Cmos 将应用、框架和插件都称为加载单元（loadUnit），因为在代码结构上几乎没有什么差异，下面是目录结构
...
在加载过程中，Cmos 会遍历所有的 loadUnit 加载上述的文件（应用、框架、插件各有不同），加载时有一定的优先级
...
plugin1 为 framework1 依赖的插件，配置合并后 object key 的顺序会优先级于 plugin2/plugin3。因为 plugin2 和 plugin3 的依赖关系，所以交换了位置。framework1 继承了 cmos，顺序会晚于 cmos。应用最后加载。

### 文件顺序

上面已经列出了默认会加载的文件，cmos 会按如下文件顺序加载，每个文件或目录再根据 loadUnit 的顺序去加载（应用、框架、插件各有不同）。

- 加载 [plugin](./plugin.md)，找到应用和框架，加载 `config/plugin.js`
- 加载 [config](../基础层/config.md), 遍历 loadUnit 加载 `config/config.{env}.js`
- 加载 [extend](../基础层/extend.md), 遍历 loadUnit 加载 `app/extend/xx.js`
- [自定义初始化](../基础层/app-start.md)，遍历 loadUnit 加载 `app.js` 和 `agent.js`
- 加载 [model](../基础层/model.md), 遍历 loadUnit 加载 `app/model` 目录
- 加载 [service](../基础层/service.md), 遍历 loadUnit 加载 `app/service` 目录
- 加载 [middleware](../基础层/middleware.md), 遍历 loadUnit 加载 `app/middleware` 目录
- 加载 [controller](../基础层/controller.md), 加载应用的 `app/controller` 目录
- 加载 [router](../基础层/router.md), 加载应用的 `app/router.js`
...
Cmos 基于 Loader 实现了 [AppWorkerLoader] 和 [AgentWorkerLoader]，上层框架基于这两个类来扩展，**Loader 的扩展只能在框架进行**。

```js
// 自定义 AppWorkerLoader
// lib/loader.js
const AppWorkerLoader = require('cmos').AppWorkerLoader;
...
// index.js
// 拷贝一份 Cmos 的 API
Object.assign(exports, require('cmos'));
